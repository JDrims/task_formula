...
import_files()
{
    ...
    then
        mkdir -p "$import"
        git clone --depth 1 http://vt-gitlab.ckba.local/fpga_education/openhwgroup_cvw.git "$import/original/cvw"
    fi
...
prompt_to_clone_if_repo_not_found()
{
    ...
    then
        ...
        printf "Needed files are located at http://vt-gitlab.ckba.local/fpga_education/openhwgroup_cvw.git\n"
